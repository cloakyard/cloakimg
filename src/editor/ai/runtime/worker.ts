...
// still caches the *model* (42–168 MB ISNet ONNX) which is the part
// actually worth caching for offline use.
...
function friendlyErrorMessage(raw: string): string {
  ...
  // OOM / allocation failure — possible on the larger ISNet tiers
  // (84 MB / 168 MB) on memory-starved devices.
  if (
    ...
    return "This device ran out of memory loading the model. Try the Fast (~42 MB) tier, or switch to the Chroma keyer for flat backgrounds.";
  }
