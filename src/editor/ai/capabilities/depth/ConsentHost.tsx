...
// modal. Subscribes to the depth service and renders the generic
// CapabilityConsentModal while status === "needs-consent".
//
// Unlike the same-origin face model, depth is a transformers.js model
// behind the HF CacheStorage probe, so the modal gets a real
...
export function DepthConsentHost() {
  ...
  const onAccept = useCallback(
    (tierId: string) => {
      ...
      // subscribed during the modal resolves on the first ready.
      depth.grantConsent();
