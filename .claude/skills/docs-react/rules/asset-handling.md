...
await sharp("input.svg").resize(800, 940).png({ compressionLevel: 6 }).toFile("output.png");
...
const resolvedUri = resolveLocalUri(mockupUri.replace("{{color}}", selectedColor.id));
