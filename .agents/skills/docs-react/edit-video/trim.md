...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-create-video-trim-browser/index.html)
...
class Example implements EditorPlugin {
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    const sampleVideo = await engine.block.addVideo(videoUri, blockWidth, blockHeight);
    ...
    const totalDuration = engine.block.getDouble(videoFill, "fill/video/totalDuration");
    // eslint-disable-next-line no-console
    ...
    const basicTrimVideo = await engine.block.addVideo(videoUri, blockWidth, blockHeight);
    ...
    // eslint-disable-next-line no-console
    console.log(`Basic trim - Offset: ${currentOffset}s, Length: ${currentLength}s`);
    ...
    const durationTrimVideo = await engine.block.addVideo(videoUri, blockWidth, blockHeight);
    ...
    // eslint-disable-next-line no-console
    console.log("Trim+Duration - Block will play trimmed 5s exactly once in timeline");

    // Create a video block with trim + looping
    const loopingTrimVideo = await engine.block.addVideo(videoUri, blockWidth, blockHeight);
    ...
    const frameAccurateTrimVideo = await engine.block.addVideo(videoUri, blockWidth, blockHeight);
    ...
    for (let i = 0; i < batchVideoUris.length; i++) {
      const batchVideo = await engine.block.addVideo(batchVideoUris[i], blockWidth, blockHeight);
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.log("Video trim guide initialized. Click play button to start playback.");
    }
  }
}
...
The _trim_ controls what plays from the source media, while the _duration_ controls how long that playback appears in the composition. If the duration exceeds the trim length and if looping is disabled, the trimmed portion will play once and then hold the last frame for the remaining duration.
