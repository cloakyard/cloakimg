...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-open-the-editor-import-design-from-indesign-browser/index.html)
...
import { IDMLParser, addGfontsAssetLibrary, createPdfEmbeddedImporter } from "@imgly/idml-importer";
...
class Example implements EditorPlugin {
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    const importIdml = async (source: string | ArrayBuffer, sourceName: string) => {
      ...
      console.log(`IDML imported successfully with ${loadedPages.length} page(s)`);
    ...
    cesdk.actions.register(
      "importScene",
      async ({ format = "scene" }: { format?: "scene" | "archive" | "idml" }) => {
        if (format === "idml") {
