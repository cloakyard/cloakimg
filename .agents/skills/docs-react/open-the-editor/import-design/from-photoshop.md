...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-open-the-editor-import-design-from-photoshop-browser/index.html)
...
import type { AssetQueryData, AssetResult, AssetsQueryResult } from "@cesdk/engine";
import type { TypefaceResolver } from "@imgly/psd-importer";
import { PSDParser, addGfontsAssetLibrary, createWebEncodeBufferToPNG } from "@imgly/psd-importer";
...
const PSD_BASE_URL = "https://staticimgly.com/imgly/docs-reference-files-temp/psd-template-import";
...
class Example implements EditorPlugin {
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    const customFontResolver: TypefaceResolver = async (fontParams, eng) => {
      ...
      const matchingFont =
        typeface.fonts.find(
          (f: { weight?: string; style?: string }) => f.weight === weight && f.style === style,
        ) ||
    ...
    const importPsd = async (source: string | ArrayBuffer, sourceName: string) => {
    ...
    engine.asset.addSource({
      ...
      async findAssets(queryData: AssetQueryData): Promise<AssetsQueryResult<AssetResult>> {
    ...
    const filteredDockOrder = existingDockOrder.filter((entry) => entry.key !== "ly.img.templates");
    ...
    cesdk.actions.register(
      "importScene",
      async ({ format = "scene" }: { format?: "scene" | "archive" | "psd" }) => {
        if (format === "psd") {
