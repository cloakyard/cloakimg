...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-export-save-publish-export-partial-export-browser/index.html)
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    cesdk.actions.register("exportDesign", async () => {
      ...
      if (selectedBlocks.length > 0) {
        ...
        blockToExport =
          selectedBlocks.length === 1 ? selectedBlocks[0] : engine.block.group(selectedBlocks);
        // eslint-disable-next-line no-console
        console.log(
      ...
      // eslint-disable-next-line no-console
      console.log(`✨ Export complete - size: ${(blob.size / 1024).toFixed(2)} KB`);
    ...
    const exportIndividualBlock = async () => {
      ...
      await downloadWithNotification(individualBlob, "block-export.png", "image/png", "Block");
    };
    ...
    const exportGroupExample = async () => {
      ...
      // eslint-disable-next-line no-console
      console.log(`✅ Group exported - size: ${(groupBlob.size / 1024).toFixed(2)} KB`);

      // Close the export dialog
      exportDialog.close();

      // Download the exported group
      await downloadWithNotification(groupBlob, "group-export.png", "image/png", "Group");
    };
    ...
    const exportCurrentPage = async () => {
      ...
      // eslint-disable-next-line no-console
      console.log(`✅ Page exported - size: ${(pageBlob.size / 1024).toFixed(2)} KB`);

      // Close the export dialog
      exportDialog.close();

      // Download the exported page
      await downloadWithNotification(pageBlob, "page-export.png", "image/png", "Page");
    };
...
// eslint-disable-next-line no-console
console.log(`✅ Individual block exported - size: ${(individualBlob.size / 1024).toFixed(2)} KB`);
