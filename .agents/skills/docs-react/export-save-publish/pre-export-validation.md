...
![Pre-Export Validation example showing validation workflow](https://img.ly/docs/cesdk/./assets/browser.hero.webp)
...
>
> - [View source on GitHub](https://github.com/imgly/cesdk-web-examples/tree/release-$UBQ_VERSION$/guides-export-save-publish-export-pre-export-validation-browser)
>
...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-export-save-publish-export-pre-export-validation-browser/index.html)
...
```typescript file=@cesdk_web_examples/guides-export-save-publish-export-pre-export-validation-browser/browser.ts reference-only
...
interface ValidationIssue {
  type: "outside_page" | "protruding" | "text_obscured" | "unfilled_placeholder";
...
class Example implements EditorPlugin {
  ...
  private async createDemoScene(engine: CreativeEngine, page: number): Promise<void> {
    ...
    engine.block.setPositionX(obscuredText, startX + (elementWidth + spacing) * 2);
    ...
    engine.block.setPositionX(overlappingShape, startX + (elementWidth + spacing) * 2);
  ...
  private getRelevantBlocks(engine: CreativeEngine): number[] {
    return [...engine.block.findByType("text"), ...engine.block.findByType("graphic")];
  }

  private findOutsideBlocks(engine: CreativeEngine, page: number): ValidationIssue[] {
  ...
  private findProtrudingBlocks(engine: CreativeEngine, page: number): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    const pageBounds = this.getBoundingBox(engine, page);
  ...
  private findObscuredText(engine: CreativeEngine, page: number): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  private isPlaceholderFilled(engine: CreativeEngine, blockId: number): boolean {
  ...
  private overrideExportAction(cesdk: CreativeEditorSDK, engine: CreativeEngine): void {
