...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-import-media-from-remote-source-pexels-browser/index.html)
...
const fetchFromPexels = async (url: string, apiKey: string): Promise<PexelsApiResponse> => {
...
const createPexelsApi = (apiKey: string) => {
  return {
    photos: {
      ...
      curated: async ({ per_page, page }: { per_page?: number; page?: number }) => {
        const params = new URLSearchParams();
        if (per_page) params.append("per_page", per_page.toString());
        if (page) params.append("page", page.toString());
