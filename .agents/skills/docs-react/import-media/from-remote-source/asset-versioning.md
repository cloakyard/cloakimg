...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-import-media-from-remote-source-asset-versioning-browser/index.html)
    ...
    for (const blockId of graphicBlocks) {
      ...
      if (fillType === "//ly.img.ubq/fill/image") {
        ...
        if (uri.includes("old-cdn.example.com")) {
          const migratedUri = uri.replace("old-cdn.example.com", "new-cdn.example.com");
          engine.block.setString(blockFill, "fill/image/imageFileURI", migratedUri);
        }
      }
    }
