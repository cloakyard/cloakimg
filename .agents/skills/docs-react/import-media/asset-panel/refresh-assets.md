...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-import-media-asset-library-refresh-assets-browser/index.html)
...
import type { AssetsQueryResult, EditorPlugin, EditorPluginContext } from "@cesdk/cesdk-js";
    ...
    engine.asset.addSource({
      ...
      async findAssets(queryData): Promise<AssetsQueryResult> {
        ...
        const filteredAssets = externalAssets.filter(
          (asset) =>
            !queryData.query || asset.name.toLowerCase().includes(queryData.query.toLowerCase()),
        );
...
```typescript
const widget = cloudinary.createUploadWidget({ cloudName: "my-cloud" }, (error, result) => {
