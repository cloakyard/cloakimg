...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-create-composition-blend-modes-browser/index.html)
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        engine.block.setString(backgroundFill, "fill/image/imageFileURI", baseImageUrl);
        ...
        engine.block.setString(overlayFill, "fill/image/imageFileURI", overlayImageUrl);
