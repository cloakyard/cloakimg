...
**Navigation:** [Starter Kits](./starterkits.md) > [Extensibility](./starterkits/extensibility.md) > [Design Validation](./starterkits/design-validation.md)
...
> **Resources:**
>
> - [Download examples](https://github.com/imgly/starterkit-design-validation-react-web/archive/refs/tags/release-$UBQ_VERSION$.zip)
>
> - [View source on GitHub](https://github.com/imgly/starterkit-design-validation-react-web/tree/v$UBQ_VERSION$)
>
> - [Open in StackBlitz](https://stackblitz.com/github/imgly/starterkit-design-validation-react-web/tree/v$UBQ_VERSION$)
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/starterkit-design-validation/index.html)
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    <TerminalTabs>
      ...
      <TerminalTab label="degit">
        npx degit imgly/starterkit-design-validation-react-web starterkit-design-validation-react-web
      </TerminalTab>
    </TerminalTabs>
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the Design Validation Editor into an existing React application. This adds the editor configuration to your current project structure.
    ...
    <TerminalTabs>
      <TerminalTab label="git">
        ...
        rm -rf starterkit-design-validation-react-web
      </TerminalTab>

      <TerminalTab label="degit">
        npx degit imgly/starterkit-design-validation-react-web/src/imgly ./src/imgly
      </TerminalTab>
    </TerminalTabs>
    ...
    export default function DesignEditor() {
      return (
        <CreativeEditor
          ...
          init={initDesignValidationEditor}
          width="100vw"
          height="100vh"
        />
      );
...
```typescript title="src/imgly/validation.ts"
import type CreativeEditorSDK from "@cesdk/cesdk-js";
import type { BlockValidationResult } from "./types";

// Validates blocks that are completely outside the page
export function validateOutsideBlocks(cesdk: CreativeEditorSDK): BlockValidationResult[];

// Validates blocks that partially protrude from the page
export function validateProtrudingBlocks(cesdk: CreativeEditorSDK): BlockValidationResult[];

// Validates text blocks that may be obstructed by other blocks
export function validatePartiallyHiddenTexts(cesdk: CreativeEditorSDK): BlockValidationResult[];
...
Each function returns an array of `BlockValidationResult` objects:

```typescript title="src/imgly/types.ts"
...
```typescript title="src/app/Sidebar.tsx"
import {
...
// Define validation metadata at the app level
const VALIDATIONS = [
  {
  ...
  },
  {
    ...
    description: "Some text elements are partially obstructed by other elements.",
    validate: validatePartiallyHiddenTexts,
  },
  {
    name: "Low resolution",
    description: "Some elements have a low resolution.",
    validate: validateLowResolution,
  },
];

// Run all validations
async function runValidationChecks(cesdk: CreativeEditorSDK) {
  const allResults = [];
