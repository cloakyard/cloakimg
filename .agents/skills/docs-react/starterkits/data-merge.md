...
**Navigation:** [Starter Kits](./starterkits.md) > [Creative Automation](./starterkits/creative-automation.md) > [Data Merge](./starterkits/data-merge.md)
...
>
> - [View source on GitHub](https://github.com/imgly/starterkit-batch-image-generation-react-web/tree/v$UBQ_VERSION$)
>
> - [Open in StackBlitz](https://stackblitz.com/github/imgly/starterkit-batch-image-generation-react-web/tree/v$UBQ_VERSION$)
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/starterkit-batch-image-generation/index.html)
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    Start fresh with a standalone Data Merge project. This creates a complete, ready-to-run React application.
    ...
    <TerminalTabs>
      ...
      <TerminalTab label="degit">
        npx degit imgly/starterkit-batch-image-generation-react-web starterkit-batch-image-generation-react-web
      </TerminalTab>
    </TerminalTabs>

    The `src/` folder contains the batch image generation code:
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the Data Merge into an existing React application. This adds the editor configuration to your current project structure.
    ...
    <TerminalTabs>
      <TerminalTab label="git">
        git clone https://github.com/imgly/starterkit-batch-image-generation-react-web.git
        cp -r starterkit-batch-image-generation-react-web/src/imgly ./src/imgly
        rm -rf starterkit-batch-image-generation-react-web
      </TerminalTab>

      <TerminalTab label="degit">
        npx degit imgly/starterkit-batch-image-generation-react-web/src/imgly ./src/imgly
      </TerminalTab>
    </TerminalTabs>
        ...
        export default function DesignEditor() {
          return (
            <CreativeEditor
              ...
              init={initBatchImageGenerationTemplateEditor}
              width="100vw"
              height="100vh"
            />
          );
      ...
      <TabItem label="Instance Editor">
        ...
        export default function DesignEditor() {
          return (
            <CreativeEditor
              config={{ baseURL: '/assets' }}
              init={initBatchImageGenerationInstanceEditor}
              width="100vw"
              height="100vh"
            />
          );
...
The Data Merge starterkit uses headless batch rendering to generate personalized images at scale without displaying a UI. This is powered by the `@cesdk/engine` package, which provides a lightweight rendering engine optimized for automated workflows.

### How Batch Rendering Works

The `batchRender` function is a self-contained utility that handles the complete rendering lifecycle:

1. **Call batchRender** with a scene string, array of items, and options
2. **Engine lifecycle is automatic** — the function creates an engine, renders all items, and disposes it
3. **Receive results** with blobs and scene strings for each item

```typescript title="src/imgly/batch-renderer.ts"
import { batchRender } from "./imgly";

// Render a single item - engine is created and disposed automatically
const [result] = await batchRender(
  templateSceneString,
  [
    {
      images: { Photo: "https://example.com/john-doe.jpg" },
      variables: {
        FirstName: "John",
        LastName: "Doe",
...
const modifiedScene = result.sceneString;
```

### Processing Multiple Records

Generate images for multiple data records in a single batch call:

```typescript title="src/app/App.tsx"
import { batchRender } from "./imgly";

const employees = [
  { firstName: "John", lastName: "Doe", department: "Engineering", photo: "john.jpg" },
  { firstName: "Jane", lastName: "Smith", department: "Marketing", photo: "jane.jpg" },
];

// Build batch items from employee data
const items = employees.map((emp) => ({
  images: { Photo: `/images/${emp.photo}` },
  variables: {
...
// Render all employees - engine lifecycle is handled internally
const results = await batchRender(templateSceneString, items, {
  baseURL: "/assets",
  mimeType: "image/png",
  // license: 'YOUR_LICENSE_KEY',
});
...
results.forEach((result, index) => {
  const imageUrl = URL.createObjectURL(result.blob);
  // Use the generated image (display, download, upload to server, etc.)
});
```
...
interface BatchResult {
  ...
  sceneString: string; // The scene with applied variables (for re-editing)
}
```

The `sceneString` preserves all applied variables and images, allowing users to re-open and edit individual results in the UI editor.
...
The Data Merge starterkit uses asset source plugins to provide built-in libraries for templates, stickers, shapes, and fonts. The starter kit includes a curated selection—customize what's included based on your needs.
...
The Data Merge starterkit includes everything needed for batch personalization and headless rendering.

<CapabilityGrid
features={[
{
title: 'Headless Batch Rendering',
description:
'Generate personalized designs at scale using the headless engine with variable replacement and image swapping.',
imageId: 'transform',
},
{
title: 'Role-Based Editing',
description:
'Two distinct editing modes: Creator role for template design and Adopter role for limited instance editing.',
imageId: 'filters',
},
{
title: 'Modal-Based Editor',
description:
'Open CE.SDK editors in a full-screen modal overlay with automatic cleanup and resource management.',
imageId: 'green-screen',
},
{
title: 'Variable System',
description:
'Use CE.SDK variables to personalize templates with dynamic content like names, departments, and data fields.',
imageId: 'text-editing',
},
{
title: 'Dual SDK Integration',
description:
'Combines @cesdk/engine for headless rendering and @cesdk/cesdk-js for full UI editing in a single application.',
imageId: 'asset-libraries',
},
{
title: 'Batch Progress Tracking',
description:
'Monitor batch rendering progress with callbacks and handle errors gracefully during high-volume generation.',
imageId: 'client-side',
},
]}
/>
