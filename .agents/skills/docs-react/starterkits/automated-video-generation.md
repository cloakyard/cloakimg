...
**Navigation:** [Starter Kits](./starterkits.md) > [Video Editing](./starterkits/video-editing.md) > [Automated Video Generation](./starterkits/automated-video-generation.md)
...
> **Resources:**
>
> - [Download examples](https://github.com/imgly/starterkit-video-placeholders-react-web/archive/refs/tags/release-$UBQ_VERSION$.zip)
>
> - [View source on GitHub](https://github.com/imgly/starterkit-video-placeholders-react-web/tree/v$UBQ_VERSION$)
>
> - [Open in StackBlitz](https://stackblitz.com/github/imgly/starterkit-video-placeholders-react-web/tree/v$UBQ_VERSION$)
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/starterkit-video-placeholders/index.html)
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    <TerminalTabs>
      ...
      <TerminalTab label="degit">
        npx degit imgly/starterkit-video-placeholders-react-web starterkit-video-placeholders-react-web
      </TerminalTab>
    </TerminalTabs>
    ...
    │   │   └── video-editor/          # Same structure as advanced-video-editor/
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the Automated Video Generation into an existing React application. This adds the editor configuration to your current project structure.
    ...
    <TerminalTabs>
      <TerminalTab label="git">
        git clone https://github.com/imgly/starterkit-video-placeholders-react-web.git
        cp -r starterkit-video-placeholders-react-web/src/imgly ./src/imgly
        rm -rf starterkit-video-placeholders-react-web
      </TerminalTab>

      <TerminalTab label="degit">
        npx degit imgly/starterkit-video-placeholders-react-web/src/imgly ./src/imgly
      </TerminalTab>
    </TerminalTabs>
    ...
    <Tabs syncKey="editor-mode">
      <TabItem label="Adopter">
        ...
        import { initVideoPlaceholdersAdopterEditor } from './imgly';

        export default function DesignEditor() {
          return (
            <CreativeEditor
              config={{ baseURL: '/assets' }}
              init={initVideoPlaceholdersAdopterEditor}
              width="100vw"
              height="100vh"
            />
          );
      ...
      <TabItem label="Creator">
        ```tsx
        import CreativeEditor from '@cesdk/cesdk-js/react';
        import { initVideoPlaceholdersCreatorEditor } from './imgly';

        export default function DesignEditor() {
          return (
            <CreativeEditor
              config={{ baseURL: '/assets' }}
              init={initVideoPlaceholdersCreatorEditor}
              width="100vw"
              height="100vh"
            />
          );
...
export async function initVideoPlaceholdersCreatorEditor(cesdk: CreativeEditorSDK) {
  // Use advanced video editor for full editing capabilities
...
```typescript title="src/imgly/index.ts"
export async function initVideoPlaceholdersAdopterEditor(cesdk: CreativeEditorSDK) {
  // Use standard video editor for simplified experience
...
- `exportVideo` – Export the current video to MP4 or other formats
...
await cesdk.actions.run("exportVideo", { mimeType: "video/mp4" });
```
...
cesdk.actions.register("exportVideo", async (exportOptions) => {
