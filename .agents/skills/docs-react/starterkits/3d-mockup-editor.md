...
**Navigation:** [Starter Kits](./starterkits.md) > [Creative Automation](./starterkits/creative-automation.md) > [3D Mockup Editor](./starterkits/3d-mockup-editor.md)

---

Integrate CE.SDK with 3D libraries for real-time, interactive design previews on products. Enhance accuracy and boost conversions with lifelike visuals.
...
> **Resources:**
>
> - [Download examples](https://github.com/imgly/starterkit-3d-product-preview-react-web/archive/refs/tags/release-$UBQ_VERSION$.zip)
>
> - [View source on GitHub](https://github.com/imgly/starterkit-3d-product-preview-react-web/tree/v$UBQ_VERSION$)
>
> - [Open in StackBlitz](https://stackblitz.com/github/imgly/starterkit-3d-product-preview-react-web/tree/v$UBQ_VERSION$)
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/starterkit-3d-product-preview/index.html)
...
<Tabs syncKey="project-type">
  <TabItem label="New Project">
    ...
    Start fresh with a standalone 3D Product Configurator project. This creates a complete, ready-to-run React application.
    ...
    <TerminalTabs>
      ...
      <TerminalTab label="degit">
        npx degit imgly/starterkit-3d-product-preview-react-web starterkit-3d-product-preview-react-web
      </TerminalTab>
    </TerminalTabs>

    The `src/` folder contains the React application and CE.SDK integration:
  ...
  <TabItem label="Existing Project">
    ...
    Integrate the 3D Mockup Editor into an existing React application. This adds the editor configuration to your current project structure.
    ...
    <TerminalTabs>
      <TerminalTab label="git">
        git clone https://github.com/imgly/starterkit-3d-product-preview-react-web.git
        cp -r starterkit-3d-product-preview-react-web/src/imgly ./src/imgly
        rm -rf starterkit-3d-product-preview-react-web
      </TerminalTab>

      <TerminalTab label="degit">
        npx degit imgly/starterkit-3d-product-preview-react-web/src/imgly ./src/imgly
      </TerminalTab>
    </TerminalTabs>
    ...
    import { init3dProductPreviewEditor } from './imgly';

    export default function DesignEditor() {
      return (
        <CreativeEditor
          config={{ baseURL: '/assets' }}
          init={init3dProductPreviewEditor}
          width="100vw"
          height="100vh"
        />
      );
...
The 3D Product Configurator uses a unique dual-engine architecture for real-time mockup rendering:
...
│ • History management    │     │ • Exports texture PNG   │
└─────────────────────────┘     └─────────────────────────┘
                                           │
                                           ▼
                              ┌─────────────────────────┐
...
4. The rendered texture is applied to the 3D model via Google Model Viewer
...
// result.mockupUrl - Blob URL of rendered mockup (texture for 3D model)
...
The 3D Product Configurator uses asset source plugins to provide built-in libraries for templates, stickers, shapes, and fonts. The starter kit includes a curated selection—customize what's included based on your needs.
...
The 3D Product Configurator includes everything needed for professional design editing and interactive 3D product visualization.
...
description:
'Design changes are instantly rendered onto the 3D model using a headless CE.SDK engine for real-time texture updates.',
imageId: 'transform',
},
{
title: 'Multiple Product Types',
description:
'Supports Business Cards, Baseball Caps, and T-Shirts out of the box, with easy extensibility for custom products.',
imageId: 'filters',
},
{
title: 'Interactive 3D Models',
description:
'Rotate, zoom, and inspect products from any angle using Google Model Viewer with GLTF/GLB support.',
...
description:
'Download the rendered texture as PNG to use in external 3D workflows or production systems.',
imageId: 'client-side',
},
]}
/>
...
### 3D model doesn't update

- **Check engine initialization**: Ensure both the design SDK and headless engine are initialized
- **Verify mockup scene loading**: The mockup scene must load successfully before rendering
- **Check placeholder names**: Placeholders in mockup scenes must match the naming pattern (e.g., "Image 1")
