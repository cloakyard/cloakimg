...
**Navigation:** [Starter Kits](./starterkits.md) > [Assets](./starterkits/assets.md) > [Layouts Editor](./starterkits/layouts-editor.md)

---

Allow users to select different layouts without changing page content.

![Layouts Editor starter kit showing a professional layout-based editing interface](https://img.ly/docs/cesdk/./assets/browser.hero.webp)

> **Reading time:** 10 minutes
>
> **Resources:**
>
> - [Download examples](https://github.com/imgly/starterkit-layouts-asset-source-ts-web/archive/refs/tags/release-$UBQ_VERSION$.zip)
>
> - [View source on GitHub](https://github.com/imgly/starterkit-layouts-asset-source-ts-web/tree/release-$UBQ_VERSION$)
>
> - [Open in StackBlitz](https://stackblitz.com/github/imgly/starterkit-layouts-asset-source-ts-web/tree/release-$UBQ_VERSION$)
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/starterkit-layouts-asset-source/index.html)
  ...
  <TabItem label="New Project">
    ...
    <TerminalTabs>
      <TerminalTab label="git">
        ...
        rm -rf starterkit-layouts-asset-source-ts-web
      </TerminalTab>

      <TerminalTab label="degit">
        npx degit imgly/starterkit-layouts-asset-source-ts-web/src/imgly ./src/imgly
      </TerminalTab>
    </TerminalTabs>
    ...
    export default function LayoutsEditor() {
      return (
        <CreativeEditor
          config={{ baseURL: '/assets' }}
          init={initLayoutsAssetSource}
          width="100vw"
          height="100vh"
        />
      );
...
await cesdk.engine.asset.addLocalAssetSourceFromJSONString(JSON.stringify(LAYOUT_ASSETS), baseURL);
```

The `addLocalAssetSourceFromJSONString()` method registers the layout templates as an asset source. The JSON file contains layout definitions with `{{base_url}}` placeholders that are automatically replaced with the configured base URL at runtime.
...
The `applyLayoutToPage()` function handles the layout application by:

1. Loading the layout scene from the asset URI
2. Replacing the current page's structure with the new layout
3. Copying text content and images from the old page to corresponding blocks in the new layout
4. Preserving fonts, colors, and image source sets during the transfer
...
- `meta.thumbUri`: Path to the thumbnail image
