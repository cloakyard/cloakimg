...
```ts
type UnsupportedCapabilityBehavior = "block" | "warn" | "ignore";
```

Behavior for a browser capability check at editor startup.

- `'block'`: Show a blocking error dialog (no dismiss)
- `'warn'`: Show a dismissible warning dialog
