...
| `at?`    | `A` _extends_ `"ly.img.dock"` ? [`DockPosition`](./api/cesdk-js/type-aliases/dockposition.md) : `never` | Dock position: `'left'`, `'right'`, or `'bottom'`. Defaults to `'left'`. Only available for `'ly.img.dock'`. |
