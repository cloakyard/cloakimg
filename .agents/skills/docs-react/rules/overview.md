...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-rules-overview-browser/index.html)
...
class Example implements EditorPlugin {
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    engine.block.setScopeEnabled(appearanceBlock, "appearance/adjustments", false);
    ...
    for (let i = 0; i < labels.length; i++) {
      ...
      engine.block.replaceText(label, `${labels[i].text}\n${labels[i].constraint}`);
    ...
    const canMoveLayer = engine.block.isAllowedByScope(layerBlock, "layer/move");
    const canMoveEnabled = engine.block.isAllowedByScope(enabledBlock, "layer/move");
    const canMoveLocked = engine.block.isAllowedByScope(lockedBlock, "layer/move");
