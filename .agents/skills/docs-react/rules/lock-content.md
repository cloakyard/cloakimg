...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-rules-lock-content-browser/index.html)
...
```typescript file=@cesdk_web_examples/guides-rules-lock-content-browser/browser.ts reference-only
...
class Example implements EditorPlugin {
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    // eslint-disable-next-line no-console
    console.log("Text editing enabled for:", engine.block.getName(editableText));
    ...
    // eslint-disable-next-line no-console
    console.log("Image replacement enabled for:", engine.block.getName(replaceableImage));
    ...
    // eslint-disable-next-line no-console
    console.log("Position adjustment enabled for:", engine.block.getName(movableShape));
  ...
  private logPermissions(engine: CreativeEngine, name: string, blockId: number): void {
...
```typescript highlight=highlight-find-all-scopes
// Get all available scopes
const allScopes = engine.editor.findAllScopes();
// eslint-disable-next-line no-console
console.log("Available scopes:", allScopes);
...
```typescript highlight=highlight-lock-all
// Step 1: Lock everything by setting all global scopes to 'Deny'
for (const scope of allScopes) {
  engine.editor.setGlobalScope(scope, "Deny");
}
...
```typescript highlight=highlight-text-edit
// Step 2: Allow text editing on specific blocks
// Set text/edit scope to 'Defer' so block-level settings take effect
engine.editor.setGlobalScope("text/edit", "Defer");
engine.editor.setGlobalScope("text/character", "Defer");
...
```typescript highlight=highlight-image-replace
// Step 3: Allow image replacement on specific blocks
// Set fill/change scope to 'Defer'
engine.editor.setGlobalScope("fill/change", "Defer");
...
```typescript highlight=highlight-position-adjust
// Step 4: Allow repositioning of specific blocks
// Set layer/move scope to 'Defer'
engine.editor.setGlobalScope("layer/move", "Defer");
engine.editor.setGlobalScope("layer/resize", "Defer");
...
```typescript highlight=highlight-log-permissions
