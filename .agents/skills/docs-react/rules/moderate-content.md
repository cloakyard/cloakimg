...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-rules-moderate-content-browser/index.html)
...
class Example implements EditorPlugin {
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    engine.block.setString(textBlock, "text/text", "Sample text content for moderation testing");
  ...
  private async demonstrateContentModeration(engine: CreativeEngine): Promise<void> {
    ...
    if (failed.length > 0) {
      ...
      engine.block.findAllSelected().forEach((id) => engine.block.setSelected(id, false));
  ...
  private getImageUrl(engine: CreativeEngine, blockId: number): string | null {
    try {
      ...
      const fillImageURI = engine.block.getString(imageFill, "fill/image/imageFileURI");
      if (fillImageURI) {
        return fillImageURI;
      }
   ...
   */
  private async checkImageContent(engine: CreativeEngine): Promise<ValidationResult[]> {
   ...
   */
  private async checkTextContent(engine: CreativeEngine): Promise<ValidationResult[]> {
   ...
   */
  private percentageToState(percentage: number): "success" | "warning" | "failed" {
    if (percentage > 0.8) {
      return "failed";
