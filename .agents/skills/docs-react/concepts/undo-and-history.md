...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-concepts-undo-and-history-browser/index.html)
...
class Example implements EditorPlugin {
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    console.log("Switched to secondary history. Active:", engine.editor.getActiveHistory());
    ...
    engine.editor.addUndoStep();
    console.log("Block added in secondary history. canUndo:", engine.editor.canUndo());

    // Switch back to primary history
    engine.editor.setActiveHistory(primaryHistory);
    console.log("Switched back to primary history. canUndo:", engine.editor.canUndo());
...
const secondaryHistory = engine.editor.createHistory();
const primaryHistory = engine.editor.getActiveHistory();
console.log("Created secondary history. Primary:", primaryHistory, "Secondary:", secondaryHistory);
