...
![Blocks example showing a scene with graphic and text blocks](https://img.ly/docs/cesdk/./assets/browser.hero.webp)
...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-concepts-blocks-browser/index.html)
...
class Example implements EditorPlugin {
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    console.log("Opacity readable:", isOpacityReadable, "writable:", isOpacityWritable);
    ...
    console.log("Available blend modes:", blendModes.slice(0, 3).join(", "), "...");
    ...
    const unsubscribeSelection = engine.block.onSelectionChanged(() => {
      ...
      console.log("Selection changed, now selected:", selected.length, "blocks");
    });
    ...
    const unsubscribeState = engine.block.onStateChanged([graphic], (changedBlocks) => {
      for (const blockId of changedBlocks) {
        ...
        if (state.type === "Pending" && state.progress !== undefined) {
          console.log("Loading progress:", Math.round(state.progress * 100) + "%");
        }
      }
    });
    ...
    console.log("Demonstrated: types, hierarchy, properties, selection, state, and serialization.");
  }
}
...
// Set text content
engine.block.setString(textBlock, "text/text", "Blocks are the building units of CE.SDK designs");
