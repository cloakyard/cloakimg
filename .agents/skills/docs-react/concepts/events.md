...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-concepts-events-browser/index.html)
  ...
  async initialize({ cesdk }: EditorPluginContext): Promise<void> {
    ...
    const unsubscribeAll = engine.event.subscribe([], (events) => {
      for (const event of events) {
        console.log(`[All Blocks] ${event.type} event for block ${event.block}`);
      }
    });
    ...
    const unsubscribeSpecific = engine.event.subscribe([graphic], (events) => {
      for (const event of events) {
        console.log(`[Specific Block] ${event.type} event for block ${event.block}`);
      }
    });
      ...
      for (const event of events) {
        switch (event.type) {
          ...
          case "Destroyed": {
            ...
            console.log(`Block ${event.block} destroyed, still valid: ${isValid}`);
            break;
          }
        }
      }
      ...
      for (const event of events) {
        if (event.type === "Destroyed") {
          ...
          if (engine.block.isValid(event.block)) {
            ...
            console.log(`Block ${event.block} has been destroyed and is invalid`);
          }
        }
      }
    });
    ...
    console.log("Demonstrated: subscribing, event types, processing, and cleanup.");
  }
}
