...
>
> - [Live demo](https://cdn.img.ly/demo/cesdk-web-examples/v1.79.0/examples/guides-create-composition-position-and-align-browser/index.html)
...
class Example implements EditorPlugin {
  ...
  private addNavigationButtons(cesdk: NonNullable<EditorPluginContext["cesdk"]>) {
  ...
  private async runPositionDemo(cesdk: NonNullable<EditorPluginContext["cesdk"]>) {
    const engine = cesdk.engine;
    const page = engine.block.findByType("page")[0];
  ...
  private async runDistributeDemo(cesdk: NonNullable<EditorPluginContext["cesdk"]>) {
    const engine = cesdk.engine;
    const page = engine.block.findByType("page")[0];
  ...
  private async runSnappingDemo(cesdk: NonNullable<EditorPluginContext["cesdk"]>) {
    const engine = cesdk.engine;
    const page = engine.block.findByType("page")[0];
